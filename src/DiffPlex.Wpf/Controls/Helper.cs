    ...
    internal static class Helper
    {
        ...
        internal static void RenderInlineDiffs(InternalLinesViewer viewer, DiffPaneModel m, UIElement source)
        {
            ...
            foreach (var line in m.Lines)
            {
                if (line == null)
                {
                    var c = viewer.Add("", ChangeType.Unchanged.ToString(), source);
                    c.Tag = line;
                    continue;
                }
                ...
                switch (line.Type)
                {
                    ...
                    case ChangeType.Inserted:
                        if (line.SubPieces != null && line.SubPieces.Count > 1 && !disableSubPieces)
                        {
                            ...
                            var c = viewer.Add(details, changeType.ToString(), source);
                            c.Tag = line;
                            hasAdded = true;
                        }
                ...
                if (!hasAdded)
                {
                    var c = viewer.Add(text, changeType.ToString(), source);
                    c.Tag = line;
                }
            }
        ...
        {
            // Not sure how to do this when we have FrameworkContentElement inlines now

            //var currentScrollPosition = panel.ValueScrollViewer.VerticalOffset;
            //var point = new Point(0, currentScrollPosition);
            //foreach (var item in panel.ValuePanel.Inlines)
            //{
            //    if (!(item is FrameworkContentElement ele) || !(ele.Tag is DiffPiece line) || line?.Position != lineIndex) 
            //        continue;
        ...
        private static List<KeyValuePair<string, string>> GetSubPiecesInfo(DiffPiece line, bool isOld)
        {
            ...
            foreach (var ele in line.SubPieces)
            {
                ...
                var subTypeStr = subType != ChangeType.Imaginary ? subType.ToString() : "";
                if (details.Count > 0)
                {
        ...
        private static void InsertLinesInteral(InternalLinesViewer panel, List<DiffPiece> lines, bool isOld, UIElement source, bool disableSubPieces = false)
        {
            foreach (var line in lines)
            {
                if (line == null)
                {
                    var c = panel.Add("", ChangeType.Unchanged.ToString(), source);
                    c.Tag = line;
                    continue;
                }
                ...
                switch (line.Type)
                {
                    case ChangeType.Modified:
                        ...
                        if (line.SubPieces != null && line.SubPieces.Count > 1 && !disableSubPieces)
                        {
                            ...
                            var c = panel.Add(details, changeType.ToString(), source);
                            c.Tag = line;
                            hasAdded = true;
                        }
                ...
                if (!hasAdded)
                {
                    var c = panel.Add(text, changeType.ToString(), source);
                    c.Tag = line;
                }
            }
